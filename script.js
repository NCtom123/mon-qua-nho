...
function tapLove(id) {
  ...
  if (loveTaps.size === 4) {
    Swal.fire({
      title: 'Đủ rồi thì sẵn sàng nhận quà nha! 🎁',
      ...
      typeWriterEffect(
        `Chúc bạn ${userName} ngày 1/6 thật vui vẻ như một đứa trẻ, như được trở về tuổi thơ với những ký ức ngọt ngào🤗. Dù đã lớn, nhưng ai cũng có một đứa trẻ trong tim. Nhân ngày 1/6, hãy dành tặng một nụ cười cho chính “đứa trẻ” trong bạn😘. Chúc bạn những điều tốt đẹp nhất, luôn vui vẻ và hạnh phúc và mãi giữ nụ cười trên môi nhé!😆`,
        'loveMsg',
        () => {
          const fromTag = document.createElement("div");
          fromTag.id = 'fromTag';
          fromTag.textContent = "From: Nguyễn Chí Tâm";
...
async function inipesan() {
  const { value: typedName } = await Swal.fire({
    title: 'Xin hỏi người đẹp tên là gì ấy nhờ ?',
    input: 'text',
    inputValue: '',
